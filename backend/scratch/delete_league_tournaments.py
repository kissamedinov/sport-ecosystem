...
tournaments = db.query(Tournament).filter(Tournament.name.ilike("%league%")).all()
print(f"Found {len(tournaments)} tournaments containing 'league' in name.")
...
for t in tournaments:
    ...
    if match_ids:
        print(f"  - Deleting {len(match_ids)} matches and their statistics/lineups...")
    ...
    if tteam_ids:
        print(f"  - Deleting {len(tteam_ids)} tournament teams and squads...")
    ...
    # 5. Delete from other tournament association tables
    ...
    # 6. Delete divisions and stats
    if division_ids:
        print(f"  - Deleting {len(division_ids)} divisions and stats...")
        # Delete from tournament_player_stats
        ...
        # Delete from tournament_divisions
    ...
    print(f"Successfully deleted tournament '{t.name}'.")

db.commit()
print("All matches for tournaments containing 'league' in name have been deleted from DB.")
db.close()
